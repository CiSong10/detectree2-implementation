...
from utility.utility import (safe_register_train_data,
                             ...
                             MultiBandPredictor)
from utility.cleaning import secondary_cleaning, canopy_mask_filter
from utility.clean_nms import clean_crowns, post_clean  # temporary
...
class Site:
    ...
    def tile(self):
        ...
        """
        if (
            self.tiles_dir.is_dir()
            and any(self.tiles_dir.iterdir())
            and not self.configs.force_retile
        ):
            self.logger.debug(
    ...
    def predict(self, cfg):
        ...
        cleaned_crowns = clean_crowns(
            crowns,
            self.configs.intersection,
            confidence=self.configs.confidence,
            area_threshold=self.configs.min_area,
        )
        cleaned_crowns = cleaned_crowns.set_geometry(
            cleaned_crowns.simplify(self.configs.simplify)
        )
        cleaned_crowns.to_file(
            self.crowns_out_file,
            driver="GPKG",
            layer=f"{self.name}_{self.configs.model}_clean",
        )

        # Fill in the gaps left by the cleaning
        post_cleaned_crowns = post_clean(unclean_df=crowns, clean_df=cleaned_crowns)
        post_cleaned_crowns.to_file(
            self.crowns_out_file,
            driver="GPKG",
            layer=f"{self.name}_{self.configs.model}_postclean",
        )

        self.logger.info(
            f"[{self.name}] Done predicting. Saved: {self.crowns_out_file}"
        )
    ...
    def large_prediction(self, chunk_size=10000):
        ...
        canopy_mask_path = next((self.path / "mask").glob("*.tif"))
        print(
            f"Phase 1: Found {len(geojson_files)} files. Processing in chunks of {chunk_size}..."
        )

        for i in range(0, len(geojson_files), chunk_size):
            chunk = geojson_files[i : i + chunk_size]
            print(
                f"Processing chunk {i//chunk_size + 1}/{len(geojson_files)//chunk_size + 1}"
            )
            ...
            clean = clean_crowns(
                crowns,
                self.configs.intersection,
                self.configs.confidence,
                self.configs.min_area,
            )
        ...
        final = gpd.GeoDataFrame(
            pd.concat(final_parts + [merged], ignore_index=True), crs=g.crs
        )
...
class Pipeline:
    def __init__(self, configs: Configs):
        ...
        self.sites = [
            Site(Path("data") / site, self.appends, configs) for site in configs.data
        ]
    ...
    def train(self):
        ...
        for site in self.sites:
            to_traintest_folders(
                site.tiles_dir,
                site.tiles_root,
                test_frac=self.configs.test_frac,
                folds=self.configs.folds,
    ...
    def _plot_metrics(self):
        ...
        if len(self.sites) > 1:
            for i, site in enumerate(self.sites):
                ap50_metrics = [
                    row for row in metrics if f"{site.name}_val/segm/AP50" in row
                ]
                iterations = [x["iteration"] for x in ap50_metrics]
                ap50_values = [x[f"{site.name}_val/segm/AP50"] for x in ap50_metrics]
                self.logger.info(
                    f"[{self.configs.model}] Final AP50 Value of site {site.name}: {'{:.2f}'.format(ap50_values[-1])}"
                )
            ...
            self.logger.info(
                f"[{self.configs.model}] Final AP50 Value: {'{:.2f}'.format(ap50_values[-1])}"
            )
            plt.plot(
                iterations,
                ap50_values,
    ...
    def evaluate(self, test_site: Path | str = None, resplit: bool = False):
        ...
        if not test_site:
            ...
            to_traintest_folders(
                test_site.tiles_dir,
                test_site.tiles_root,
                test_frac=1,  # everyything goes to the test_dir
                folds=self.configs.folds,
                strict=self.configs.strict,
            )
